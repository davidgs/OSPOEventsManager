...
export class MemStorage implements IStorage {
  ...
  async getEvents(): Promise<Event[]> {
    try {
      ...
      // Map database columns back to the expected format
      const mappedEvents = dbEvents.map(dbEvent => {
        console.log('Mapping event:', dbEvent.id, 'website:', dbEvent.website, 'link:', dbEvent.link);
        
        return {
          ...dbEvent,
          // Map database 'website' column back to expected 'link' field for frontend
          link: dbEvent.website || dbEvent.link,
          // Use the 'goal' column from database (which should have the array data)
          goal: dbEvent.goal || dbEvent.goals || ['attending']
        };
      });
  ...
  async createEvent(insertEvent: InsertEvent): Promise<Event> {
    ...
    // Ensure goal is stored as a JSON string if it's an array
    const processedEvent = {
      ...insertEvent,
      goal: Array.isArray(insertEvent.goal) 
        ? JSON.stringify(insertEvent.goal) 
        : insertEvent.goal
    };
    ...
    const event: Event = { 
      ...
      description: null,
      website: null,
      goals: Array.isArray(insertEvent.goal) ? insertEvent.goal : JSON.parse(insertEvent.goal as string)
    };
  ...
  async updateEvent(id: number, updateEvent: Partial<InsertEvent>): Promise<Event | undefined> {
    ...
    // Process goal array for storage
    const processedUpdate = { ...updateEvent };
    
    if (updateEvent.goal) {
      processedUpdate.goal = Array.isArray(updateEvent.goal) 
