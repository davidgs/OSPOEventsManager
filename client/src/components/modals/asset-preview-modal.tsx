...
export function AssetPreviewModal({ asset, isOpen, onClose, userName }: AssetPreviewModalProps) {
  ...
  useEffect(() => {
    if (isOpen && asset) {
      ...
      const savedAnnotations = localStorage.getItem(`asset-annotations-${asset.id}`);
      if (savedAnnotations) {
        try {
  ...
  useEffect(() => {
    if (asset && annotations.length > 0) {
      localStorage.setItem(`asset-annotations-${asset.id}`, JSON.stringify(annotations));
    }
  }, [annotations, asset]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4 flex-1 min-h-0">
            ...
            {asset.mimeType.startsWith('image/') ? (
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                <div className="text-center mt-4">
                  <p className="text-sm font-medium mb-2">{asset.mimeType.split('/')[1].toUpperCase()} file preview not available</p>
                  <Button size="sm" onClick={handleDownload}>Download to view</Button>
                </div>
              </div>
            )}
            ...
            {annotations.map((annotation) => (
              <div
                ...
                className={`absolute w-6 h-6 bg-primary text-white rounded-full flex items-center justify-center -translate-x-1/2 -translate-y-1/2 cursor-pointer transition-all ${
                  selectedAnnotation === annotation.id ? 'ring-2 ring-offset-2 ring-primary' : ''
                }`}
                style={{
                  left: `${annotation.x}%`,
                  top: `${annotation.y}%`,
          ...
          <div className="bg-muted/10 rounded-md p-4 border border-border min-h-[300px] flex flex-col">
            ...
            <div className="text-xs mb-4">
              ...
              <p className="mb-1"><span className="font-semibold">Size:</span> {formatBytes(asset.fileSize)}</p>
              {asset.description && (
                <p className="mb-1">
                  ...
                  {annotations.map((annotation, index) => (
                    <div 
                      ...
                      className={`p-2 rounded-md text-xs transition-colors ${
                        selectedAnnotation === annotation.id ? 'bg-primary/10 border border-primary/30' : 'bg-muted/20'
                      }`}
                    >
                      <div className="flex justify-between items-center mb-1">
                        <div className="flex items-center">
                          <span className="font-semibold mr-1">Note {index + 1}</span>
                      ...
                      {selectedAnnotation === annotation.id ? (
                        ...
                        <p className="whitespace-pre-wrap" onClick={() => handleAnnotationClick(annotation.id)}>
                          {annotation.text}
                        </p>
                      )}
            ...
            {isAddingAnnotation && (
              <div className="mt-4 space-y-2">
                <Label htmlFor="annotation-text" className="text-xs">New annotation text:</Label>
                <Textarea
                  id="annotation-text"
                  ...
                  className="text-xs"
                />
                <p className="text-[10px] text-muted-foreground">Click on the image to place this annotation</p>
              </div>
            )}
          </div>
        </div>
